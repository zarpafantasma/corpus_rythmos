...
# --- CONFIGURATION ---
FILE_NAME = "BTCUSDT-1m-2020-03.csv" # Ensure this file is in the same folder
...
def calculate_rolling_alpha(df, window):
    ...
    for i in range(window, len(df)):
        ...
        # Check for variance (avoid flat lines)
...
def plot_analysis(df):
    ...
    plt.title('RTM Analysis: March 2020 (Full Month)')
    plt.savefig('RTM_March2020_Full.png')
    print("Saved: RTM_March2020_Full.png")
    plt.close()

    # Plot 2: Zoom on Crash (March 10-14)
    mask = (df['Date'] >= '2020-03-10') & (df['Date'] <= '2020-03-14')
    zoom_df = df.loc[mask]
    
    fig, ax1 = plt.subplots(figsize=(12, 6))
    
    color = 'tab:blue'
    ax1.set_xlabel('Date (March 10-14)')
    ax1.set_ylabel('Price (USD)', color=color)
    ...
    plt.title('Forensic Zoom: The Moment of Collapse')
    plt.savefig('RTM_March2020_Zoom.png')
    print("Saved: RTM_March2020_Zoom.png")
    plt.close()
