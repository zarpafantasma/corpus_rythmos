...
# --- CONFIGURATION ---
FILE_NAME = "BTCUSDT-1m-2021-05.csv" # Ensure this file is in the same folder
...
def plot_analysis(df):
    ...
    plt.title('RTM Analysis: May 2021 (The China Ban)')
    plt.savefig('RTM_May2021_Full.png')
    print("Saved: RTM_May2021_Full.png")
    plt.close()

    # Plot 2: Zoom on Crash (May 18-20)
    mask = (df['Date'] >= '2021-05-18') & (df['Date'] <= '2021-05-20')
    zoom_df = df.loc[mask]
    
    fig, ax1 = plt.subplots(figsize=(12, 6))
    
    color = 'tab:blue'
    ax1.set_xlabel('Date (May 18-20)')
    ax1.set_ylabel('Price (USD)', color=color)
    ...
    plt.title('Forensic Zoom: The Moment of Panic')
    plt.savefig('RTM_May2021_Zoom.png')
    print("Saved: RTM_May2021_Zoom.png")
    plt.close()
