...
def get_noise_filter(symbol):
    """Retorna el filtro de ruido absoluto exacto según la escala del activo"""
...
def fetch_live_rtm_data(symbol='BTC/USD'):
    try:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            raw_alpha = cov / (var + 1e-9)
            
        raw_alpha = pd.Series(raw_alpha).replace([np.inf, -np.inf], np.nan)
        # Fix: Si la varianza es muy baja, Alpha se asume en su pulso base saludable (0.45)
        raw_alpha = np.where(var < 1e-7, 0.45, raw_alpha)
        
        df['Rolling_Alpha'] = pd.Series(raw_alpha).fillna(0.45).rolling(window=3, min_periods=1).mean()
...
def fetch_systemic_health():
    ...
    for sym in assets:
        try:
            ...
            alpha = 0.45 if (np.isnan(alpha) or var < 1e-7) else alpha
...
def apply_premium_layout(fig, chart_height=750):
    """Estilo institucional con etiquetas blancas y altura extendida"""
    fig.update_layout(
...
if menu == "MICROSTRUCTURE (LIVE)":
    ...
    if live_df is not None and not live_df.empty:
        ...
        <div class="rtm-info-card">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
                <div>
                    <b style="color: #00E5FF;">THE ALPHA EXPONENT (α)</b><br>
    ...
    if macro_data is not None:
        ...
        st.warning("Please ensure 'crash_alpha_analysis.csv' is in the root directory.")
    ...
    if df_f is not None:
        ...
        fig_f.add_trace(go.Scatter(x=df_f['Date'], y=df_f['Close'], name="PRICE (USD)", line=dict(color='#00E5FF', width=2), fill='tozeroy', fillcolor='rgba(0, 229, 255, 0.05)'), secondary_y=False)
        fig_f.add_trace(go.Scatter(x=df_f['Date'], y=df_f['Rolling_Alpha'], name="RTM ALPHA (α)", line=dict(color='#FF0000', width=2.2)), secondary_y=True)
        
        fig_f.add_trace(go.Scatter(x=[None], y=[None], mode='lines', line=dict(color='rgba(255, 23, 68, 0.6)', dash='dash'), name='FRACTURE (2.0)'), secondary_y=True)
        fig_f.add_trace(go.Scatter(x=[None], y=[None], mode='lines', line=dict(color='rgba(255, 234, 0, 0.6)', dash='dash'), name='VISCOSITY (1.2)'), secondary_y=True)
        
        fig_f.add_hline(y=2.0, line_dash="dash", line_color="rgba(255, 23, 68, 0.5)", secondary_y=True)
        fig_f.add_hline(y=1.2, line_dash="dash", line_color="rgba(255, 234, 0, 0.5)", secondary_y=True)
        
        if event_sel != "SEPTEMBER 2023 (CONTROL GROUP)":
            peak_idx = df_f['Rolling_Alpha'].idxmax()
            start_idx = max(0, peak_idx - 180) 
            cap_idx = min(len(df_f)-1, peak_idx + 120) 
            
            p_date = df_f.loc[peak_idx, 'Date']
            s_date = df_f.loc[start_idx, 'Date']
            c_date = df_f.loc[cap_idx, 'Date']
            
            fig_f.add_vline(x=s_date, line_dash="dot", line_color="#A0AEC0", opacity=0.6)
            fig_f.add_annotation(x=s_date, y=2.6, text="SYSTEMIC STRESS ONSET", showarrow=False, font=dict(color="#A0AEC0", size=10), xanchor="left", xshift=5, yref="y2")
            
            fig_f.add_vline(x=p_date, line_dash="solid", line_color="#FFEA00", opacity=0.8)
            fig_f.add_annotation(x=p_date, y=3.0, text="MAX STRUCTURAL ENTROPY", showarrow=False, font=dict(color="#FFEA00", size=10, weight="bold"), xanchor="left", xshift=5, yref="y2")
            
            fig_f.add_vline(x=c_date, line_dash="dot", line_color="#FF1744", opacity=0.6)
            fig_f.add_annotation(x=c_date, y=2.4, text="LIQUIDITY CASCADE", showarrow=False, font=dict(color="#FF1744", size=10), xanchor="left", xshift=5, yref="y2")
            
        fig_f = apply_premium_layout(fig_f, chart_height=750)
        fig_f.update_yaxes(title_text="", secondary_y=True, range=[-0.1, 3.1]) 
        st.plotly_chart(fig_f, use_container_width=True)
        st.markdown(event_explanations[event_sel], unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with col2_p:
            ...
            g_prob = np.exp(-sigma**2 / 2)
            r_prob = sigma ** -3
            st.write(f"Probability of a **{sigma}σ event**:")
            st.info(f"GAUSSIAN: 1 IN {int(1/g_prob):,} DAYS")
            st.success(f"RTM PHYSICS: 1 IN {int(1/r_prob):,} DAYS")
    ...
    with tab2:
        ...
        s_recovery = 365 * ((drawdown / 20.0) ** (3.59 / 2.0))
        st.metric(label="ESTIMATED RECOVERY TIME", value=f"{int(s_recovery):,} DAYS")
    ...
    with tab3:
        ...
        np.random.seed(42)
        dist_l = np.random.normal(0.45, 0.12, 18000)
        dist_t = np.random.normal(0.95, 0.15, 1500)
        dist_v = np.random.normal(1.4, 0.2, 400)
        dist_f = np.random.normal(2.1, 0.25, 50)
        dist_all = np.concatenate([dist_l, dist_t, dist_v, dist_f])
        dist_all = dist_all[(dist_all > 0.1) & (dist_all < 3.2)] 
        
        fig_h = px.histogram(dist_all, nbins=120, labels={'value': 'RTM Alpha (α)'})
        fig_h.update_traces(marker_color='#00E5FF', marker_line_color='#0B0E14', marker_line_width=1)
        fig_h.add_vline(x=1.2, line_dash="dash", line_color="#FFEA00", annotation_text="VISCOSITY", annotation_font_color="#FFEA00")
        fig_h.add_vline(x=2.0, line_dash="dash", line_color="#FF1744", annotation_text="FRACTURE", annotation_font_color="#FF1744")
        
        fig_h = apply_premium_layout(fig_h, chart_height=500)
        fig_h.update_layout(showlegend=False, yaxis_title="Frequency (Log Scale)", yaxis_type="log")
