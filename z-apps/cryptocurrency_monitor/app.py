...
def get_noise_filter(symbol):
    """Filtros de ruido calibrados para evitar colapsos matemáticos"""
...
def load_macro_data():
    ...
    if not os.path.exists(file_path):
        return None
    try:
        # Cargamos el CSV ignorando posibles espacios o comillas mal puestas
        df = pd.read_csv(file_path, skipinitialspace=True, quoting=0)
        return df
    except Exception:
        return None
...
def fetch_systemic_health():
    ...
    for sym in assets:
        try:
            ...
            health.append({"asset": sym.split('/')[0], "alpha": alpha, "price": df['c'].iloc[-1]})
        except Exception:
            health.append({"asset": sym.split('/')[0], "alpha": None, "price": None})
    return health
...
menu = st.sidebar.radio("MODULES", ("MICROSTRUCTURE (LIVE)", "MACRO EARLY WARNING", "FORENSIC LABORATORY", "MARKET PHYSICS"))

# MODULE 1: LIVE
if menu == "MICROSTRUCTURE (LIVE)":
    st.markdown("## LIVE MICROSTRUCTURE RADAR")
    col_s, col_r, _ = st.columns([1, 1, 2])
    with col_s: selected_asset = st.selectbox("ASSET", ["BTC/USD", "ETH/USD", "SOL/USD", "XRP/USD"])
    with col_r: 
        st.write(""); 
    ...
    if live_df is not None:
        ...
        with col_g:
            fig_g = go.Figure(go.Indicator(mode="gauge+number", value=curr_a, title={'text':"α EXPO"}, gauge={'axis':{'range':[None, 3]}, 'bar':{'color':"#FFFFFF"}, 'steps':[{'range':[0,0.8],'color':"rgba(0,230,118,0.15)"},{'range':[0.8,1.2],'color':"rgba(41,121,255,0.15)"},{'range':[1.2,2.0],'color':"rgba(255,234,0,0.15)"},{'range':[2,3],'color':"rgba(255,23,68,0.25)"}]}))
        ...
        with col_c:
            ...
            fig.add_trace(go.Scatter(x=live_df['Date'], y=live_df['Rolling_Alpha'], name="ALPHA (α)", line=dict(color='#FF0000')), secondary_y=True)
            st.plotly_chart(apply_layout(fig), use_container_width=True)

    st.markdown("---")
    st.markdown("#### SYSTEMIC HEALTH OVERVIEW")
    h_data = fetch_systemic_health()
    cols = st.columns(4)
    for i, h in enumerate(h_data):
        with cols[i]:
            color = "#00E676" if h['alpha'] and h['alpha'] < 0.8 else "#FFEA00" if h['alpha'] and h['alpha'] < 2.0 else "#FF1744"
            st.markdown(f'<div class="health-card" style="border-top: 3px solid {color};"><b>{h["asset"]}</b><br>α = {h["alpha"]:.3f if h["alpha"] else "N/A"}</div>', unsafe_allow_html=True)
...
# MODULE 2: MACRO
    ...
    if macro is not None:
        ...
        st.plotly_chart(apply_layout(fig_s, 500), use_container_width=True)
    else:
...
# MODULE 3: FORENSIC
    ...
    if df_f is not None:
        ...
        if ev != "SEPTEMBER 2023 (CTRL)":
            ...
            # Etiquetas horizontales escalonadas para legibilidad
            fig_f.add_vline(x=s_date, line_dash="dot", line_color="#A0AEC0")
            fig_f.add_annotation(x=s_date, y=2.4, text="STRESS ONSET", showarrow=False, xanchor="left", xshift=5, yref="y2")
            ...
            fig_f.add_annotation(x=c_date, y=2.0, text="LIQUIDITY CASCADE", showarrow=False, xanchor="left", xshift=5, yref="y2")
        
        st.plotly_chart(apply_layout(fig_f), use_container_width=True)
    else:
        st.warning(f"File '{event_dict[ev]}' not found in repository.")

# MODULE 4: PHYSICS
elif menu == "MARKET PHYSICS":
    st.markdown("## MARKET PHYSICS")
    t1, t2, t3 = st.tabs(["FAT TAILS", "RECOVERY TIME", "ALPHA DIST"])
    with t1:
        st.write("Markets follow Power Laws. Extreme events are structural features.")
        c1, c2 = st.columns(2)
        c1.metric("GLOBAL α EXPONENT", "2.966 ± 0.236")
    ...
    with t3:
        ...
        st.plotly_chart(apply_layout(fig_h, 500), use_container_width=True)
