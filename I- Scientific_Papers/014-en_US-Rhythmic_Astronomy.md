...
**Abstract**
We present Rhythmic Astronomy, an application of the RTM (Relativistic Temporal Multiscale) framework to galactic dynamics in which orbital clocks are governed not only by gravity and baryonic mass but also by a coherence exponent $\alpha$ that encodes the multiscale organization of the baryonic medium. In RTM, characteristic times scale as $T \propto L^{\alpha}$ at fixed environment; mapping this to circular orbits yields the velocity law

$v \propto r^{1 - \alpha/2}$

so that the slope of $\log v$ vs. $\log r$ inside coherence-fixed annuli equals $(1 - \alpha/2)$. This framework generates three falsifiable predictions: (i) slope tests on rotation curves binned by structural coherence, (ii) a baryonic Tully-Fisher recast in which residuals correlate with $\alpha$ -proxies rather than halo parameters, and (iii) lensing-kinematics consistency if $\alpha$ modifies operational times but not spacetime curvature.

We detail how to estimate $\alpha$ from photometric and kinematic texture --- multiscale entropy, Fourier mode power, turbulence indices --- and how to perform "collapse" checks (flatness of residuals within coherence bins) mirroring the slope-first discipline used elsewhere in the RTM corpus.

**Systematic empirical validation** $\rightarrow$ **(APPENDICES E, G, H)**. We apply this methodology to the SPARC database (Lelli et al. 2016), comprising 175 disk galaxies with Spitzer 3.6 $\mu$ m photometry and high-quality HI/H $\alpha$ rotation curves. An extensive adversarial campaign (Red Team, April 2026) produced 20 significant empirical findings across six independent analytical flanks, all controlling for total baryonic mass and/or $V_{flat}$:

*What baryonic structure predicts:*

1. **Mass discrepancy** --- baryonic concentration predicts the mass discrepancy $D = V_{obs}^2 / V_{bar}^2$ beyond what total mass explains (partial $\rho = +0.346$, $p = 0.0001$, $\Delta R^2 = 8.5\%$).
2. **Rotation curve diversity** --- SB profile slope predicts the rise-to-flat ratio at fixed $V_{flat}$ (partial $\rho = +0.329$, $p = 0.0001$), addressing the diversity problem (Oman et al. 2015).
3. **Acceleration scale non-universality** --- baryonic concentration predicts the acceleration at which DM begins to dominate, with mass having zero predictive power ($R^2_{mass} = 0.0004$, $R^2_{conc} = 0.225$; partial $\rho = -0.574$, $p = 3 \times 10^{-7}$).
4. **Baryon effectiveness** --- at fixed mass AND $V_{flat}$, concentration predicts how effectively baryons support rotation (partial $\rho = -0.446$, $p = 9.4 \times 10^{-8}$).
5. **Baryon-halo shape conspiracy** --- $V_{bar}(r)$ and $V_{DM}(r)$ shapes are correlated (mean $r = +0.274$, $p = 9.9 \times 10^{-5}$, bootstrap CI [0.14, 0.40]), and this conspiracy is modulated by gas fraction: gas-rich galaxies show $r = +0.70$, gas-poor show $r = -0.15$ ($p < 10^{-4}$).
6. **Local point-by-point coupling** --- within individual galaxies, after removing radial trends, local gas fraction predicts local inferred DM density ($\rho = -0.177$, $p = 2.5 \times 10^{-18}$, $n = 2{,}411$ radius points, cluster bootstrap CI [-0.25, -0.10]).

*What baryonic structure cannot do:*

A direct RTM velocity prediction ($v_{RTM}(r) = \kappa \cdot r^{1-\alpha(r)/2}$, 1 parameter) fails decisively against NFW halo fits (wins 2/135 galaxies). The BTFR absorbs all structural signal for $V_{flat}$ prediction ($\Delta R^2 < 0.1\%$). RAR scatter is not reduced ($< 0.1\%$).

These results establish that baryonic geometry --- especially gas geometry --- carries statistically significant information about the mass discrepancy pattern, the DM transition scale, and the baryon-halo conspiracy, but does not replace the need for additional mass or modified dynamics for absolute velocity prediction.

**Note on the flat-curve identity.** The velocity law implies $\alpha = 2(1 - \text{slope})$; for flat rotation curves, $\alpha \rightarrow 2$ follows by algebraic identity. The empirical content lies in the structural correlations, not in this mapping.

Furthermore, we extend the RTM framework to the interplanetary medium by analyzing non-collisional astrophysical plasmas $\rightarrow$ **(APPENDIX F)**, demonstrating systematic Topological Relaxation from the Iroshnikov-Kraichnan limit near the Sun to the Kolmogorov limit in deep space.
...
**2.4. Systematic Empirical Validation: The Galactic Laboratory (APPENDICES E, G, H)**

To ground these theoretical propositions in observational reality, we tested the RTM framework using the SPARC database (Lelli et al., 2016). This dataset, comprising 175 nearby disk galaxies with high-fidelity kinematics and photometry, serves as an ideal testbed. An extensive Red Team campaign (April 2026) subjected the data to six independent analytical flanks. The findings organize into two categories:

**What baryonic structure CAN do (20 significant findings):**

The geometric organization of baryonic matter --- concentration, SB slope, central surface brightness, gas fraction, profile roughness --- carries information about galactic dynamics beyond what total mass provides. Twenty independent partial correlations, all controlling for mass and/or $V_{flat}$, demonstrate this across six distinct physical quantities: mass discrepancy magnitude, rotation curve diversity, DM transition acceleration, baryon effectiveness, baryon-halo shape conspiracy, and DM growth rate. The strongest effects reach $\rho = -0.574$ ($p = 3 \times 10^{-7}$) and remain significant under bootstrap, leave-one-out, and within-$V_{flat}$ -bin replication. Gas fraction emerges as the primary modulator: gas-rich galaxies show baryon-DM shape conspiracy ($r = 0.70$) while gas-poor galaxies show none ($r = -0.15$).

A local, point-by-point analysis (2,411 radius points across 105 galaxies) reveals that within individual galaxies, after removing radial trends, local gas fraction predicts local inferred DM density ($\rho = -0.177$, $p = 2.5 \times 10^{-18}$). This is the most controlled test in the analysis: same galaxy, same radius, galaxy fixed effects removed.

**What baryonic structure CANNOT do:**

1. **Direct velocity prediction fails.** RTM with one free parameter loses 133/135 against NFW (2 parameters). The raw SB-gradient-to-$\alpha$ mapping diverges at large radii.
2. **BTFR dominance.** Structure adds $\Delta R^2 < 0.1\%$ to $V_{flat}$ prediction beyond the BTFR ($R^2 = 0.91$).
3. **RAR scatter unchanged.** Local SB gradient does not reduce RAR scatter ($< 0.1\%$).
4. **Cusp-core not predicted.** Baryonic structure cannot distinguish cuspy from cored DM profiles after mass control.

These results frame RTM's contribution precisely: baryonic geometry provides statistically significant but secondary ($\sim 4$ - $22\%$ $\Delta R^2$ depending on the quantity) predictive information about the mass discrepancy *pattern*, but does not replace the primary role of total mass in determining velocities. Full details in Appendices E, G, and H.
...
**2.6. Red Team Empirical Update: The Complete Flanking Campaign**

The original Appendix E analysis established a structure-kinematics correlation (ODR slope $= -1.17 \pm 0.12$) and reported $\alpha = 1.993 \pm 0.130$ for flat curves (an algebraic identity). Subsequent adversarial testing (Red Team, April 2026) subjected the SPARC data to six independent analytical flanks, systematically attacking the question from every angle the data permits. Appendix G covers Flanks 1-2 and the direct velocity test; Appendix H covers Flanks 3-6 and the local coupling analysis.

**Summary of the six flanks:**

1. **Mass discrepancy** (Appendix G.2): Concentration predicts $D$ at fixed mass. Partial $\rho = +0.346$, $p = 0.0001$, $\Delta R^2 = 8.5\%$.

2. **Diversity problem** (Appendix G.3): SB slope predicts rise ratio at fixed $V_{flat}$. Partial $\rho = +0.329$, $p = 0.0001$. Replicates in medium and fast rotators.

3. **Acceleration scale** (Appendix H.1): Concentration predicts the DM-transition acceleration with $R^2 = 0.225$ --- while mass has $R^2 = 0.0004$. Partial $\rho = -0.574$, $p = 3 \times 10^{-7}$. Bootstrap CI [-0.72, -0.35].

4. **Baryon effectiveness + DM growth** (Appendix H.2): At fixed mass AND $V_{flat}$, concentration predicts baryon effectiveness ($\rho = -0.446$, $p = 9.4 \times 10^{-8}$) and DM 50\% crossing radius ($\rho = -0.515$, $p = 4.2 \times 10^{-6}$).

5. **Shape conspiracy** (Appendix H.3): $V_{bar}(r)$ and $V_{DM}(r)$ shapes correlate (mean $r = +0.274$, bootstrap CI [0.14, 0.40]). Conspiracy strength is predicted by central surface brightness ($\rho = -0.326$, $p = 0.0002$).

6. **Gas fraction modulation** (Appendix H.4): Gas-rich galaxies show conspiracy $r = +0.70$; gas-poor show $r = -0.15$ ($p < 10^{-4}$). Gas fraction modulates baryon effectiveness ($\rho = -0.371$, $p = 2.4 \times 10^{-5}$) and break sharpness ($\rho = -0.235$, $p = 0.009$). The structural link (concentration $\rightarrow$ baryon effectiveness) operates only in gas-rich galaxies ($\rho = -0.416$, $p = 0.0008$) and vanishes in gas-poor ($\rho = -0.216$, $p = 0.12$).

**The local coupling** (Appendix H.5): Within individual galaxies, at the same radius, after removing galaxy identity and radial trends, local gas fraction predicts local DM density ($\rho = -0.177$, $p = 2.5 \times 10^{-18}$, 2,411 points, cluster bootstrap CI [-0.25, -0.10]).
...
(a) **Parametric monotone map (transparent):**
...
**RTM mechanism.** Inside bars/bulges/clumps, $\alpha(L) >$ `<!-- -->`{=html}1 elevates local orbital times, yielding

$$m = \frac{\partial\log v}{\partial\log L} = 1 - \alpha < 0\quad\text{(steeper rises}\text{/}\text{shallower declines depend on geometry)}.$$
...
-   **PASS (galaxy scale):**
    ...
    i.  Outer low-$\widehat{\alpha}$ bins show $m \approx 0$ **and** collapse;

    ii. $M_{kin}^{RTM}(R)$ agrees with $M_{lens}(R)$ in rings/quads within $\leq 15\%$;

    iii. Weak-lensing stacks at fixed baryons do **not** split by coherence, while kinematic slopes **do**
...
**9.3 Building** $\widehat{\mathbf{\alpha}}\mathbf{(}\mathbf{L}\mathbf{)}$
...
**10.3 Defining the coherence field** $\mathbf{\alpha}\mathbf{(x)}$
...
-   **Featureless S0/Sa disks:** proxies should yield $\hat{\alpha} \to 1$ globally; outer bins should PASS collapse with $m \approx 0$.
...
-   **Diffuse outskirts:** α→1 → flat asymptotes without invoking extra mass *if* curvature need not rise (consistent lensing is the guardrail).
...
-   **Feedback that shreds order** drives α↓ (outer disks settle faster, clumps die earlier).

-   **Long-lived coherent features** (bars, rings) drive α↑ (inner clocks slow, diversity rises).\
...
Initial OLS analysis was suppressed by **attenuation bias**. Once corrected via **Orthogonal Distance Regression (ODR)** to absorb 15% hardware and observational variance, the structure-kinematics link is revealed as a steeper slope of $\mathbf{- 1.169\ }\mathbf{\pm}\mathbf{0.119}$. Furthermore, the 52 galaxies with flat rotation curves yielded a derived coherence exponent of $\alpha = \ 1.99$. This result is algebraically expected: since $\alpha = 2(1 - \text{slope})$, flat curves (slope $\approx 0$) give $\alpha \approx 2$ by definition. The empirical content lies in the structure-kinematics correlation, not in the flat-curve mapping.

**E.2 Rigorous Probabilistic Validation (ODR & Error Propagation)**

To ensure the structure-kinematics link represents a genuine physical correlation and not a statistical artifact, the dataset was subjected to a "Red Team" statistical pipeline:
...
2.  **Monte Carlo Distribution:** For the 52 flat-curve galaxies, we simulated 52,000 data points by injecting the specific rotational velocity error margins back into the slope derivations, mapping the probabilistic distribution of the topological exponent $\alpha$.

**E.3 The Structure-Kinematics Link (Robust Findings)**

Under error propagation, the structure-kinematics correlation survives:

-   **Flat Curve Consistency:** The Monte Carlo distribution for the flat-curve galaxies yields $\mathbf{\alpha}\mathbf{= \ 1.993\ }\mathbf{\pm}\mathbf{0.130}$, consistent with the algebraic mapping $\alpha = 2(1-\text{slope})$ for near-zero slopes. This confirms internal consistency of the RTM velocity law but, as noted above, reflects a definitional relationship rather than an independent measurement.

-   **The Structure-Kinematics Link:** The robust ODR analysis confirms that the correlation between visible baryonic structure and orbital kinematics (ODR Slope $= \  - 1.169\  \pm 0.119$) survives noise injection and error propagation. This is a between-type effect: galaxies with more concentrated surface brightness profiles have systematically steeper kinematic slopes.

-   **Within-Type Analysis:** When galaxies are divided into quartiles by structural concentration and the partial correlation (controlling for $v_{max}$) is computed within each quartile, the signal weakens substantially (Q1--Q3: $\rho < 0.17$, all non-significant; Q4: $\rho = -0.29$, $p = 0.059$, marginal). This indicates that the overall correlation is driven by between-type differences rather than continuous within-type variation.

**E.4 Mass Discrepancy Analysis**

A more informative test exploits the baryonic mass model components provided by SPARC. We computed the baryonic velocity $V_{bar}^2 = |V_{gas}| \cdot V_{gas} + \Upsilon_{disk} \cdot |V_{disk}| \cdot V_{disk} + \Upsilon_{bul} \cdot |V_{bul}| \cdot V_{bul}$ (with standard mass-to-light ratios $\Upsilon_{disk} = 0.5$, $\Upsilon_{bul} = 0.7$ at 3.6 $\mu$ m) and the mass discrepancy $D = V_{obs}^2 / V_{bar}^2$ for 120 galaxies with sufficient data.

**Key finding:** At fixed total baryonic mass, the surface brightness **concentration** of the galaxy significantly predicts the outer mass discrepancy:

| Parameter | Zero-order $\rho$ | Partial $\rho$ (controlling $M_{bar}$) | $p$ |
|-----------|-------------------|---------------------------------------|-----|
| Concentration | +0.04 (ns) | **+0.346** | **0.0001** |
| SB slope | -0.02 (ns) | **-0.245** | **0.007** |

Multivariate model comparison:

| Model | $R^2$ | $\Delta R^2$ | $F$ -test $p$ |
|-------|-------|-------------|-------------|
| Mass only | 0.170 | — | — |
| Mass + structure | 0.254 | +0.085 | 0.002 |
| Structure only | 0.013 | — | — |

**Interpretation:** Baryonic structure adds statistically significant predictive power (8.5%) to the mass discrepancy beyond total baryonic mass. More concentrated galaxies exhibit larger discrepancies at fixed mass, consistent with RTM's prediction that structural geometry modulates the relationship between baryonic distribution and observed dynamics.

**Limitation:** Structure cannot predict $v_{flat}$ beyond the BTFR ($\Delta R^2 < 0.1\%$). BTFR residuals show zero correlation with structural parameters ($\rho \approx 0$). At present, the structural contribution is a secondary effect embedded within the dominant mass-velocity relationship, not a replacement for it.

**E.5 Conclusions**

The SPARC analysis demonstrates that baryonic structural geometry contains information about the mass discrepancy that total mass alone does not capture. This is consistent with RTM's core premise that topological organization of the baryonic medium modulates dynamics. However, the magnitude of the structural contribution (8.5% of variance in $D$, zero contribution to $v_{flat}$) does not yet support claims of eliminating dark matter. The question of whether refined structural proxies (multiscale entropy, Fourier mode power, kinematic texture) can substantially increase the independent contribution remains an open empirical question addressed by the proposed methodology in Sections 5--9.
...
The robust Phase 2 analysis shows that the solar wind index is not a static constant, but a measure of **Topological Relaxation**. The index evolves radially from $\mathbf{- 1.52}$ (Near-Sun Rigid Topology at 0.1 AU) to $\mathbf{- 1.72}$ (Deep Space Fractal Fluid at 2.0 AU). This radial evolution is well-documented in heliophysics (Chen 2016, Adhikari 2017, Shi 2021); RTM's contribution is the interpretation within a unified topological framework.
...
To robustly test the RTM framework, we analyzed the radial evolution of the spectral index from 0.1 AU (Parker Solar Probe) out to 2.0 AU (Ulysses). The variance-corrected trajectory demonstrates that the plasma undergoes a macroscopic **Topological Relaxation**:

-   **Near-Sun Rigid Topology (0.1 AU):** In the immediate vicinity of the Sun, intense magnetic fields enforce a rigid, highly coherent 1D-like hierarchy. The empirical spectral index here converges to $- 1.52$, consistent with the Iroshnikov-Kraichnan (IK) theoretical limit ($- 3\text{/}2$).

-   **Deep Space Fractal Fluid (1.0 - 2.0 AU):** As the plasma expands and the global magnetic field weakens, the rigid topological constraint breaks down. The plasma \"relaxes,\" fracturing into a 3D isotropic state. The spectral index drops to $- 1.68$ to $- 1.72$, consistent with the Kolmogorov fractal turbulence limit ($- 5\text{/}3$).

The linear regression of this relaxation (slope = $- 0.18$ per decade AU, $R^{2} = 0.98$) demonstrates that the spectral shift is systematic, consistent with the mathematical signature of decaying multiscale coherence as predicted by RTM.
...
**Conclusion:** The solar wind behaves as a dynamically relaxing topological network. The mapping of the plasma's evolution from the Iroshnikov-Kraichnan limit to the Kolmogorov limit is consistent with RTM's prediction that topological coherence governs energy transport in non-collisional media. This radial evolution is independently documented in the heliophysics literature; RTM's contribution is the unified interpretation through the coherence exponent framework.

### APPENDIX G: Red Team Analysis — Mass Discrepancy, Diversity Problem, and Direct Velocity Prediction

**G.1 Methodology**

All analyses use the SPARC table2.dat dataset (Lelli et al. 2016) with standard mass-to-light ratios $\Upsilon_{disk} = 0.5\ M_\odot/L_\odot$ and $\Upsilon_{bul} = 0.7\ M_\odot/L_\odot$ at 3.6 $\mu$ m. The baryonic velocity is computed as $V_{bar}^2 = |V_{gas}| \cdot V_{gas} + \Upsilon_{disk} \cdot |V_{disk}| \cdot V_{disk} + \Upsilon_{bul} \cdot |V_{bul}| \cdot V_{bul}$. Surface brightness is converted to real intensity via $I = 10^{-0.4 \cdot SB_{disk}}$. All scripts are publicly available and reproduce results with fixed random seeds.

**G.2 Mass Discrepancy Analysis**

The mass discrepancy $D(r) = V_{obs}^2(r) / V_{bar}^2(r)$ quantifies how much additional mass (or modified dynamics) is needed at each radius. We computed $D_{outer}$ as the median discrepancy in the outermost 40% of each galaxy's radial extent, where dark matter dominance is expected.

We defined two structural parameters from the SB profile: (a) **Concentration** $= \log_{10}(I_{inner} / I_{outer})$, where inner and outer refer to the innermost and outermost 30% of radii, and (b) **SB slope** $= d\log I / d\log r$ from OLS regression.

**Results:**

| Predictor | Zero-order $\rho$ with $\log D_{outer}$ | Partial $\rho$ (controlling $M_{bar}$) | $p$ |
|-----------|----------------------------------------|---------------------------------------|-----|
| Concentration | +0.04 (ns) | **+0.346** | **0.0001** |
| SB slope | $-$ 0.02 (ns) | **$-$ 0.245** | **0.007** |

The zero-order correlations are null because mass and structure are confounded. Once total baryonic mass is controlled, both structural parameters significantly predict the discrepancy: more concentrated galaxies exhibit larger mass discrepancies at fixed mass.

**Multivariate regression:**

| Model | $R^2$ | $\Delta R^2$ | $F$ -test $p$ |
|-------|-------|-------------|-------------|
| $\log M_{bar}$ only | 0.170 | — | — |
| $\log M_{bar}$ + concentration + SB slope | 0.254 | +0.085 | 0.002 |

Structure adds 8.5% of explained variance, significant at $p = 0.002$.

**G.3 The Diversity Problem**

The "rotation curve diversity problem" (Oman et al. 2015) concerns the observation that galaxies with similar $V_{flat}$ (and hence similar inferred halo mass) show markedly different inner rotation curve shapes. Standard $\Lambda$ CDM requires halo concentration scatter to account for this, but the predicted scatter is narrower than observed.

We tested whether SB profile shape predicts rotation curve shape at fixed $V_{flat}$. The **rise-to-flat ratio** is defined as $R_{rise} = V_{inner} / V_{flat}$, where $V_{inner}$ is the median velocity in the innermost 40% of radii. The **inner slope** is $d\log V / d\log r$ computed over the same range.

**Partial correlations (controlling for $V_{flat}$), $n = 131$ galaxies:**

| Correlation | Partial $\rho$ | $p$ | Significant? |
|-------------|---------------|-----|-------------|
| SB slope $\rightarrow$ rise ratio | **+0.329** | **0.0001** | Yes (★★★) |
| SB slope $\rightarrow$ inner slope | $-$ 0.166 | 0.059 | Marginal |
| SB conc $\rightarrow$ inner slope | $-$ 0.046 | 0.60 | No |

**Within-$V_{flat}$ terciles:**

| Bin | $n$ | $\rho$ (SB slope, rise ratio) | $p$ |
|-----|-----|------------------------------|-----|
| Slow (16–83 km/s) | 44 | +0.226 | 0.14 (ns) |
| Medium (83–163 km/s) | 43 | +0.320 | 0.037 (★) |
| Fast (168–331 km/s) | 44 | +0.348 | 0.021 (★) |

The effect replicates in medium and fast rotators but not in dwarfs, where SB profiles are too diffuse to provide meaningful structural contrast.

**Multivariate model for inner slope:**

| Model | $R^2$ | $\Delta R^2$ | $F$ -test $p$ |
|-------|-------|-------------|-------------|
| $V_{flat}$ only | 0.326 | — | — |
| $V_{flat}$ + SB concentration + SB slope | 0.365 | +0.039 | 0.022 |

Baryonic structure adds 3.9% of explained variance to inner RC shape prediction beyond mass, significant at $p = 0.022$.

**Interpretation.** At fixed mass, the baryonic light distribution partially predicts rotation curve diversity. Galaxies with steeper SB profiles (more centrally concentrated light) have faster inner rotation relative to their flat portion. This is consistent with RTM's prediction that the topological organization of the baryonic medium modulates dynamics, and addresses a specific observational puzzle (Oman et al. 2015) that $\Lambda$ CDM does not naturally explain from baryonic properties alone.

**G.4 Direct Velocity Prediction Test**

As the most demanding test, we attempted to predict $V_{obs}(r)$ at each radius using only the SB intensity profile and one free parameter:

$$v_{RTM}(r) = \kappa \cdot r^{1 - \alpha(r)/2}, \quad \alpha(r) = |d\log I / d\log r|$$

where $I(r) = 10^{-0.4 \cdot SB_{disk}(r)}$ and $\kappa$ is fitted per galaxy via $\chi^2$ minimization against $V_{obs}$ with observational errors.

**Comparison against NFW and baryons-only:**

| Model | Free parameters | Median RMS | Wins (head-to-head vs NFW) |
|-------|----------------|------------|---------------------------|
| Baryons only ($V_{bar}$) | 0 | 42.7% | — |
| **RTM** ($\kappa$) | **1** | **80.5%** | **2/135 (1.5%)** |
| NFW halo ($V_{200}$, $c$) | 2 | 11.9% | 133/135 (98.5%) |

RTM loses decisively. The raw SB gradient $|d\log I/d\log r|$ diverges at large radii (exponential disk profile), producing $\alpha \gg 2$ and driving predicted velocities toward zero — the opposite of a flat rotation curve.

**Conclusion.** The current SB-gradient-to-$\alpha$ mapping is too crude to generate competitive velocity predictions. The success of RTM in galactic dynamics lies in the secondary structural correlations (Sections G.2 and G.3), not in direct curve prediction with this mapping.

**G.5 RAR Scatter Analysis**

We tested whether local SB structure reduces the scatter of the Radial Acceleration Relation (McGaugh et al. 2016). Using 3,174 radius points across 141 galaxies, the baseline RAR scatter is $\sigma = 0.184$ dex. After correcting for local SB gradient and magnitude, the scatter reduces to $\sigma = 0.184$ dex — a reduction of 0.1%, which is negligible.

Within-galaxy analysis shows a weak but significant tendency: mean within-galaxy $\rho$ (SB gradient, RAR residual) $= +0.107$, with 64% of galaxies showing positive correlation ($t$ -test $p = 0.01$). The effect is real but too small to be operationally useful.

**G.6 Summary of Red Team Findings**

| Finding | Effect | $p$ | Verdict |
|---------|--------|-----|---------|
| Concentration $\rightarrow$ mass discrepancy (partial) | $\rho = +0.346$ | 0.0001 | **Genuine, \Delta R^2 = 8.5\%** |
| SB slope $\rightarrow$ rise ratio at fixed $V_{flat}$ | $\rho = +0.329$ | 0.0001 | **Genuine, diversity problem** |
| Direct $v(r)$ prediction vs NFW | RTM wins 2/135 | — | **RTM fails** |
| RAR scatter reduction | 0.1% | — | **Negligible** |
| BTFR residuals vs structure | $\rho \approx 0$ | $> 0.7$ | **No signal** |
| $\alpha = 2$ for flat curves | By definition | — | **Tautological** |

**Bottom line.** Baryonic structure contains real, statistically significant information about the mass discrepancy pattern and rotation curve diversity that total mass alone does not capture. This information operates at the $\sim 4$ – $8.5\%$ level of explained variance — secondary but genuine. It does not replace dark matter halos for velocity prediction but establishes that the geometric organization of visible matter is not dynamically irrelevant, consistent with RTM's core premise.

**APPENDIX H: Extended Red Team Analysis --- Flanks 3-6 and Local Coupling**

This appendix presents findings from four additional analytical flanks and a point-by-point local coupling analysis, extending the Red Team results in Appendix G. All analyses use SPARC table2.dat (Lelli et al. 2016) with $\Upsilon_{disk} = 0.5 \; M_\odot/L_\odot$, $\Upsilon_{bul} = 0.7 \; M_\odot/L_\odot$ at 3.6 $\mu$ m. Baryonic velocity: $V_{bar}^2 = |V_{gas}| \cdot V_{gas} + \Upsilon_{disk} \cdot |V_{disk}| \cdot V_{disk} + \Upsilon_{bul} \cdot |V_{bul}| \cdot V_{bul}$. All scripts publicly available with fixed random seeds.

**H.1 Flank 3: The Acceleration Scale Is Not Universal**

McGaugh et al. (2016) established the Radial Acceleration Relation with a characteristic scale $g_\dagger \approx 1.2 \times 10^{-10}$ m/s $^2$, assumed universal. We tested whether this scale varies systematically with baryonic structure.

For each galaxy, we found $g_{cross}$ --- the baryonic acceleration at which the mass discrepancy $D = V_{obs}^2 / V_{bar}^2$ first exceeds 2.0 (where DM equals baryons in force contribution). $n = 68$ galaxies.

**Key finding:** Mass has zero predictive power for $g_{cross}$ ($R^2 = 0.0004$). Baryonic concentration alone explains 22.5% of its variance ($R^2 = 0.225$).

| Metric | Value |
|--------|-------|
| Partial Spearman $\rho$ (concentration $\rightarrow$ $g_{cross}$, controlling $M_{bar}$) | **-0.574** |
| $p$ -value | **3.06\times10^{-7}** |
| Bootstrap 95% CI | **[-0.720, -0.351]** |

More concentrated galaxies transition to DM dominance at lower accelerations. The effect is stable across D thresholds:

| D threshold | $n$ | Partial $\rho$ | $p$ |
|-------------|-----|----------------|-----|
| 1.5 | 51 | -0.447 | 0.001 |
| 2.0 | 68 | -0.574 | $3 \times 10^{-7}$ |
| 3.0 | 73 | -0.484 | $1.5 \times 10^{-5}$ |
| 5.0 | 54 | -0.304 | 0.026 |

**Theoretical implications.** Standard $\Lambda$ CDM expects $g_\dagger$ to be universal. This finding is in tension. MOND predicts $g_\dagger$ should vary with surface density; our concentration measure is a surface density proxy. RTM predicts structural coherence modulates dynamics. All three interpretations generate testable consequences.

**Caveat.** The effect is strongest in slow rotators (dwarfs, $\rho = -0.456$, $p = 0.029$) and weakens in massive galaxies. This is physically expected: dwarfs are where DM-transition effects are most prominent.


**H.2 Flank 4: Baryon Effectiveness, DM Growth Rate, and Break Sharpness**

**H.2.1 Baryon effectiveness.** "Baryon effectiveness" $= V_{bar,max} / V_{flat}$ measures how much of the observed rotation baryons can account for at their peak contribution. Partial correlations controlling for BOTH $M_{bar}$ AND $V_{flat}$ ($n = 131$):

| Predictor | Partial $\rho$ | $p$ | Bootstrap 95% CI |
|-----------|----------------|-----|-------------------|
| Concentration | **-0.446** | **9.4 \times 10^{-8}** | [-0.602, -0.247] |
| $\mu_0$ (central SB) | **+0.450** | **7.0 \times 10^{-8}** | [+0.262, +0.603] |

At fixed mass AND velocity, more concentrated galaxies are less efficient at converting baryons into rotational support. Brighter-center galaxies are more efficient.

**H.2.2 DM 50% crossing radius.** At which radius does $f_{DM} = V_{DM}^2/V_{obs}^2$ first reach 0.5? Partial correlations controlling for $M_{bar}$ ($n = 71$):

| Predictor | Partial $\rho$ | $p$ |
|-----------|----------------|-----|
| Concentration | **-0.515** | **4.2 \times 10^{-6}** |
| $\mu_0$ | **+0.470** | **3.6 \times 10^{-5}** |
| SB slope | +0.266 | 0.025 |

More concentrated galaxies reach DM dominance at smaller radii.

**H.2.3 DM growth rate.** How fast does $f_{DM}$ grow with $\log r$? Partial correlations controlling for $M_{bar}$ + $V_{flat}$ ($n = 113$):
...
**H.2.4 Break sharpness.** How abruptly does the baryon-to-DM transition occur? ($n = 113$, controlling $M_{bar}$ + $V_{flat}$):

| Predictor | Partial $\rho$ | $p$ |
|-----------|----------------|-----|
| Concentration | **-0.373** | **4.7 \times 10^{-5}** |
| $\mu_0$ | **+0.326** | **0.0004** |
| SB roughness | **+0.328** | **0.0004** |

More concentrated galaxies show gentler (NFW-like) transitions. Fainter or rougher profiles show sharper (MOND-like) transitions. Structure adds $\Delta R^2 = 11.6\%$ to break sharpness prediction ($F$ -test $p = 0.003$).


**H.3 Flank 5: The Baryon-Halo Shape Conspiracy**

For each galaxy, we normalized $V_{bar}(r)$ and $V_{DM}(r) = \sqrt{V_{obs}^2 - V_{bar}^2}$ to unit amplitude and computed the Pearson correlation between their shapes ($n = 123$).

| Metric | Value |
|--------|-------|
| Mean shape correlation | **+0.274** |
| $t$ -test vs 0 | **$t = 4.03$, p = 9.9 \times 10^{-5}** |
| Bootstrap 95% CI | **[0.138, 0.402]** |
| Bootstrap % positive | **100%** |

In $\Lambda$ CDM, the DM halo forms independently of baryonic distribution. Its radial profile should not correlate with $V_{bar}(r)$ shape after accounting for mass. The observed correlation ($r = 0.274$, bootstrap-confirmed) is the baryon-halo conspiracy, quantified.

**What predicts conspiracy strength?** Partial correlations controlling for $M_{bar}$ + $V_{flat}$:
...
Smooth, faint-center galaxies show the strongest conspiracy. Concentrated or bumpy profiles weaken it.

**Derivative conspiracy (null).** Gradients of $V_{bar}$ and $V_{DM}$ do not correlate (mean $r = 0.009$, CI includes zero). The conspiracy is in broad radial envelopes, not point-to-point wiggles, ruling out trivial mathematical artifacts.


**H.4 Flank 6: Gas Fraction as the Hidden Modulator**

SPARC provides $V_{gas}$ and $V_{disk}$ separately. Gas and stars have fundamentally different geometries: gas extends further, fills the potential uniformly, and is pressure-supported. Stars are concentrated in a thin, collisionless disk.

**The headline finding:**

| Galaxy type | Mean conspiracy $r$ | $n$ |
|-------------|---------------------|-----|
| Gas-rich ($f_{gas} > 0.18$) | **+0.705** | 61 |
| Gas-poor ($f_{gas} \leq 0.18$) | **-0.151** | 62 |
| Mann-Whitney $p$ | **< 10^{-4}** | |

The conspiracy we found in H.3 (mean $r = 0.274$) is an average of two distinct populations. Gas-rich galaxies show tight baryon-DM shape coupling. Gas-poor galaxies show none.

**Gas fraction as independent structural predictor** (partial, controlling $M_{bar}$ + $V_{flat}$):

| $f_{gas}$ $\rightarrow$ | Partial $\rho$ | $p$ |
|--------------------------|----------------|-----|
| Conspiracy strength | **+0.279** | **0.002** |
| Baryon effectiveness | **-0.371** | **2.4 \times 10^{-5}** |
| Break sharpness | **-0.235** | **0.009** |

**Critical interaction:** The concentration $\rightarrow$ baryon effectiveness link operates only in gas-rich galaxies:

| Subsample | $\rho$ (conc $\rightarrow$ baryon eff, controlling $M$ +$V$) | $p$ |
|-----------|-------------------------------------------------------------|-----|
| Gas-rich ($n = 61$) | **-0.416** | **0.0008** |
| Gas-poor ($n = 52$) | -0.216 | 0.12 (ns) |

**Physical interpretation.** Gas fills the gravitational potential uniformly (pressure-supported), extends beyond the stellar disk, and responds to the total mass structure. Stars are frozen collisionless artifacts. When gas dominates, baryonic geometry couples to dynamics; when stars dominate, mass alone determines kinematics. This explains why structural effects appear in some galaxies and not others, and suggests that RTM's "structural coherence" is primarily mediated by gas.

**Combined model:** $M + V_{flat}$ + concentration + $\mu_0$ + $f_{gas}$ + roughness predicts 50.2% of conspiracy variance ($R^2$), up from 39.9% with mass alone. $\Delta R^2 = +0.103$, $F$ -test $p = 4.6 \times 10^{-4}$. Bootstrap $\Delta R^2$ 95% CI = [0.049, 0.208], excludes zero.


**H.5 The Final Assault: Local Point-by-Point Coupling**

All previous flanks operated at the galaxy level. This analysis goes inside the galaxies. At each radius in each galaxy, we computed local inferred DM density ($\rho_{DM}(r) \propto (1/r^2) \cdot d(r \cdot V_{DM}^2)/dr$) and local baryonic geometry (SB gradient, gas fraction, SB excess). We then removed galaxy fixed effects AND radial trends, asking: at fixed radius within the same galaxy, does local baryonic composition predict local DM density?

**Result (2,411 radius points, 105 galaxies):**

| Local coupling | Spearman $\rho$ | $p$ | Cluster bootstrap 95% CI |
|----------------|-----------------|-----|--------------------------|
| $f_{gas}$ $\rightarrow$ $\rho_{DM}$ | **-0.177** | **2.5 \times 10^{-18}** | **[-0.251, -0.097]** |
| $d\log I/d\log r$ $\rightarrow$ $\rho_{DM}$ | **+0.103** | **4.4 \times 10^{-7}** | **[+0.039, +0.165]** |

Both bootstrap CIs exclude zero (cluster bootstrap by galaxy, 3,000 iterations).

**Per-galaxy consistency:** Mean within-galaxy $\rho(f_{gas}, \rho_{DM} | \text{radius}) = -0.147$, 67% negative, $t$ -test $p = 3.2 \times 10^{-4}$.

**SB excess split:** Bumps in the light profile predict local DM density in gas-rich galaxies ($\rho = +0.234$, $p = 5 \times 10^{-6}$) but not in gas-poor ($\rho = -0.088$, ns). Difference: $p < 10^{-4}$.

**Galaxy fixed-effects regression:** Adding local geometry (SB gradient, SB excess, $f_{gas}$) to galaxy FE + radius yields $\Delta R^2 = +0.006$, $F = 16.3$, $p = 1.8 \times 10^{-10}$. The dominant geometry coefficient is $f_{gas} = -0.336$.

**Interpretation.** At the same radius in the same galaxy, where gas locally dominates the baryonic budget, less DM is inferred. This is the most controlled test possible with these data. The effect size is small ($\rho \approx 0.10$ -$0.18$) but massively significant ($p < 10^{-10}$), consistent across 67% of individual galaxies, and confirmed by cluster bootstrap.

**Caveat.** Part of this signal may reflect differential measurement quality: $V_{gas}$ (from 21-cm emission) is measured more directly than $V_{disk}$ (requiring assumed $\Upsilon$). Where gas dominates, $V_{bar}$ is better known, so the DM residual is mechanically smaller. The SB excess finding (bumps predict DM only in gas-rich galaxies) is harder to explain by measurement quality alone, as SB bumps are stellar features.


**H.6 Cumulative Evidence: All Structural Findings**

| \# | Finding | Partial $\rho$ | $p$ | Controls |
|----|---------|----------------|-----|----------|
| 1 | Baryon effectiveness vs concentration | -0.446 | $10^{-7}$ | $M + V$ |
| 2 | Baryon effectiveness vs $\mu_0$ | +0.450 | $10^{-7}$ | $M + V$ |
| 3 | Acceleration scale vs concentration | -0.574 | $10^{-7}$ | $M$ |
| 4 | DM 50% radius vs concentration | -0.515 | $10^{-6}$ | $M$ |
| 5 | DM 50% radius vs $\mu_0$ | +0.470 | $10^{-5}$ | $M$ |
| 6 | Break sharpness vs concentration | -0.373 | $10^{-5}$ | $M + V$ |
| 7 | $f_{gas}$ $\rightarrow$ baryon effectiveness | -0.371 | $10^{-5}$ | $M + V$ |
| 8 | Mass discrepancy vs concentration | +0.346 | $10^{-4}$ | $M$ |
| 9 | Diversity (rise ratio) vs SB slope | +0.329 | $10^{-4}$ | $V$ |
| 10 | Break sharpness vs $\mu_0$ | +0.326 | $10^{-4}$ | $M + V$ |
| 11 | Conspiracy strength vs $\mu_0$ | -0.326 | $10^{-4}$ | $M + V$ |
| 12 | Break sharpness vs roughness | +0.328 | $10^{-4}$ | $M + V$ |
| 13 | Mirror quality vs $\mu_0$ | -0.313 | $10^{-4}$ | $M + V$ |
| 14 | DM growth rate vs SB slope | +0.310 | $10^{-3}$ | $M + V$ |
| 15 | RAR offset vs $R_{eff}$ | -0.294 | $10^{-3}$ | $M + V$ |
| 16 | $f_{gas}$ $\rightarrow$ conspiracy | +0.279 | 0.002 | $M + V$ |
| 17 | Conspiracy strength vs roughness | -0.255 | 0.004 | $M + V$ |
| 18 | $f_{gas}$ $\rightarrow$ break sharpness | -0.235 | 0.009 | $M + V$ |
| 19 | Conspiracy exists (mean $r = 0.274$) | --- | $10^{-4}$ | --- |
| 20 | Gas-rich conspiracy = 0.70 vs gas-poor = -0.15 | --- | $< 10^{-4}$ | --- |
| 21 | **Local $f_{gas}$ $\rightarrow$ local $\rho_{DM}$ ** | **-0.177** | **10^{-18}** | **Galaxy FE + radius** |
...
|------|--------|---------|
| Direct $v(r)$ prediction vs NFW | RTM wins 2/135 | **RTM fails** |
| BTFR residuals vs structure | $\rho \approx 0$, $p > 0.7$ | **No signal** |
| RAR scatter reduction | 0.1% | **Negligible** |
| Cusp-core prediction from SB | Partial $\rho < 0.13$ (ns) | **Not predicted** |
| $\alpha = 2$ for flat curves | By algebraic identity | **Tautological** |

**Bottom line.** The front door is closed: RTM cannot replace NFW for direct velocity prediction. But 21 significant findings, all controlling for mass, spanning galaxy-level correlations ($\rho$ up to -0.574) to local point-by-point coupling ($\rho = -0.177$ over 2,411 points), demonstrate that baryonic geometry --- especially gas geometry --- is dynamically non-trivial. The baryon-halo conspiracy is real, quantified, and predicted by structural parameters. Gas fraction is the hidden modulator that determines when and where structural effects emerge. These findings are consistent with RTM's core claim that topological organization of the baryonic medium modulates dynamics, and they address specific open problems in galactic dynamics (diversity, baryon-halo conspiracy, $g_\dagger$ universality) that $\Lambda$ CDM does not naturally explain from baryonic properties alone.

---

*All analyses reproducible. Scripts: rtm_flank_attack.py, rtm_flank3.py, rtm_flank4.py, rtm_flank5.py, rtm_flank6.py, rtm_final_assault.py. Data: SPARC table2.dat (Lelli et al. 2016).*
