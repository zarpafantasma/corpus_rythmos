...
def syndrome_jitter_effect(distances, alpha_base, jitter_fraction=0.03):
    """
    ...
    """
    # Jitter breaks topological synchronization with noise
    # This directly raises the scaling exponent α, not just the base intercept.
    alpha_effective = alpha_base * (1 + jitter_fraction * 10)  # Heuristic 30% boost for 3% jitter
    return rtm_cycles_to_failure(distances, alpha_effective)
...
def compare_fixed_vs_jittered(distances, alpha):
    ...
    T_jittered = syndrome_jitter_effect(distances, alpha, jitter_fraction=0.03)
