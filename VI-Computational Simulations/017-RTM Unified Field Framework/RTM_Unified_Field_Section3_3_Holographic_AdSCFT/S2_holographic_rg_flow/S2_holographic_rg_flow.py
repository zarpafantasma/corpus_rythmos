...
def find_fixed_points(alpha_range, n_points=1000):
    ...
    Updated by Red Team: Handles exact zero crossings (<= 0).
    """
    ...
    for i in range(1, len(alphas)):
        # Check for sign change OR an exact hit on zero
        if betas[i-1] * betas[i] <= 0:  
            
            # If we hit zero exactly, prevent division by zero in interpolation
            if betas[i] == 0:
                a_fp = alphas[i]
            elif betas[i-1] == 0:
                a_fp = alphas[i-1]
            else:
                # Standard linear interpolation for a crossing
                a_fp = alphas[i-1] - betas[i-1] * (alphas[i] - alphas[i-1]) / (betas[i] - betas[i-1])
            
            # Prevent appending the same root twice due to overlapping loop checks
            if not any(np.isclose(a_fp, fp, atol=1e-4) for fp in fixed_points):
