![](media/image1.jpeg){width="3.7628871391076117in" height="3.3894838145231847in"}
...
-   **Temporal scale** $T$**:** for each window and band peak $f^{\text{\textbackslash*}}$, set $T = 1/f^{\text{\textbackslash*}}$(seconds). If multiple peaks per band, pre-register a centroid rule or dominance criterion.
...
**Source-modeled wavelength** $\mathbf{(}\mathbf{\lambda}\mathbf{)}$
...
*"Piecewise regimes in \[system\]. Breakpoint at* $L^{\text{\textbackslash*}}$*(CI). Slopes:* ${\widehat{\alpha}}_{1}$*(CI),* ${\widehat{\alpha}}_{2}$*(CI). Outcome: **MULTI-REGIME**. Segmentation pre-registered; collapse holds within each segment."*

**Table row fields:** system \| window \| $n_{1},n_{2}$\| ${span}_{1}$, ${span}_{2}$ \| $L^{\text{*}}$(CI) \| ${\widehat{\alpha}}_{1}$(CI), ${\widehat{\alpha}}_{2}$(CI) \| outcome = MULTI-REGIME.
