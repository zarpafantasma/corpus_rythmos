...
-   $dt_s$ and $dt_l$ represent time intervals in the small and large systems respectively
...
| :--- |
| In the context of RTM diffusions with scale-dependent conductivity $\mathcal{D}(x) = L(x)^{-\alpha}$, the effective dynamic exponent satisfies $z = m + \alpha$, where $m$ is the spatial dimension. This does not contradict the conceptual distinction between $z$ and $\alpha$: $z$ remains a phenomenological observable while $\alpha$ is the structural invariant. The relation $z = m + \alpha$ emerges as a *consequence* of the architecture, not as an identification. See Doc 002, Theorem 6.2 for the self-similarity derivation and Proposition 6.5 for exit-time scaling. |
...
graph TD
    QM["<b>[QM Region]</b>"] --> MD["<b>[MD Region]</b>"]
    MD --> CG["<b>[CG Region]</b>"]

    Atoms[Atoms] -.-> QM
    Molecules[Molecules] -.-> MD
    Networks[Macromolecules / Networks] -.-> CG

    style QM fill:#f9f,stroke:#333,stroke-width:2px
    style MD fill:#bbf,stroke:#333,stroke-width:2px
    style CG fill:#dfd,stroke:#333,stroke-width:2px
...
| Simulation | Topology | $\alpha$ Theory | $\alpha$ Measured | $R^2$ | Status |
| :--- | :--- | :--- | :--- | :--- | :---: |
