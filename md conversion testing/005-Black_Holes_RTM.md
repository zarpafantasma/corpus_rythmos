...
-   $\mathbf{1 +}\mathbf{z}\mathbf{(}\mathbf{r}\mathbf{)}$: gravitational redshift factor mapping local to asymptotic clocks.

-   $\mathbf{\Theta}\left( \mathcal{T} \right),\mathbf{\ }\mathbf{\rho}$: optional dimensionless modifiers (temperature, density) suppressed unless needed.
...
**3.4 Why "**$\mathbf{T}\mathbf{=}\mathbf{0}$**" and "**$\mathbf{\alpha}\mathbf{= \infty}$**" are not needed (and not claimed)**
...
**Setup.** $\alpha_{eff}$ **constant** (e.g., **≈2**, diffusive) across all radii; include $\mathcal{Z(}\mathbf{r}\mathbf{)}$ variation.
